...
function App() {
  ...
  const [exportDpr, setExportDpr] = useState(1); // Exact DPR for 4K
  ...
  const runSnapshot = async (canvas) => {
    try {
      ...
      a.href = dataUrl;
      a.download = `flag-snapshot-${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.png`;
      a.click();
    } catch (err) {
  ...
  const runExportLoop = async (canvas) => {
    ...
    try {
      ...
      if (isExportingRef.current) {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `flag-animation-${exportDuration}s-4k.webm`;
        a.click();
      ...
      <div className="app-container">
        ...
        {isRecording && exportMode === 'render' && (
          <div className="recording-overlay">
            <h3 style={{ margin: 0 }}>Mengekspor Video (4K)...</h3>
